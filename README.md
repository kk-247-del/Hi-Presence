...
Full E2EE: All text and voice streams are secured with End-to-End Encryption. Keys are generated at the moment of instantiation and exist only in volatile memory (RAM).
...
No Presence, No State: If required participants are not foregrounded and active, no surface, stream, or buffer exists.

No Send Primitive: There is no send() or submit(). Interaction exists only as express() or escalate().

No Persistent Objects: Text and voice exist only as live streams. Nothing is serializable beyond the Moment.

Immediate Collapse: Any violation of presence—a screen lock or backgrounding the app—triggers an immediate and irreversible teardown.

Seamless Escalation: Transitioning from text to voice is an unmute of an "always-warm" E2EE pipeline, not a new session request.

Current Functional Status (Phase 1 & 2)
